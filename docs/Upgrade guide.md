...
- [<span style="color:red">Breaking change</span>] Removed HTTPFormUsage.RawJSon support. There's a [small example](/1. HTTP Request/Advanced Topics/Small Code-Samples.md#send-json-data) on how a request can be set up to send json with the RawData property.
...
Documentation about the new and changed streaming API can be found in the [SignalR Core topics](/6. SignalR Core/1 The HubConnection Class.md#streaming-from-the-server). 